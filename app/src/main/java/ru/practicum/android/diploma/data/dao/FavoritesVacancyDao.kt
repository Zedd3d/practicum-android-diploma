...
interface FavoritesVacancyDao {
    ...
    @Query("DELETE FROM favorites_vacancies_table WHERE isFavorite = 0 AND id = :vacancyID")
    suspend fun deleteVacancyFromFavorite(vacancyID: String)

    @Query("SELECT * FROM favorites_vacancies_table WHERE isFavorite = 1 ORDER BY inDbTime DESC")
